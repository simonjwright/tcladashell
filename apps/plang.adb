...
procedure PLang is --  PLang
   ...
   begin --  Show_Usage
      ...
      New_Line;
      Put_Line
        ("   It recognizes the following languages based on file name");
      Put_Line ("   extension:");
      New_Line;
      ...
      New_Line;
      Put_Line
        ("   If the file name matches the regular expression pattern,");
      ...
      Put_Line
        ("   If found, then the language is determined by the program");
      ...
      Put_Line ("Output:");
      Put_Line
        ("   Each input file name is printed to standard output along");
   ...
   -------------------------------------------------------
   function Get_Program (Line : in String) return String;
   ...
   begin --  Get_Program
      ...
      -------------------------------------------------
         Program_Path_Name : constant String :=
            Tash.Lists.Head
              (Tash.Lists.Split (Line (Line'First + 2 .. Line'Last), " "));
      begin
   ...
   --------------------------------------------------------------------
   procedure Print_Programming_Language (File_Name : in String);
   procedure Print_Programming_Language (File_Name : in String) is
      ...
      if Tash.File.Is_Directory (File_Name) then
         if Print_Why then
            Ada.Text_IO.Put_Line
              (File_Name & ": skipped because is a directory");
         end if;
         return;
      end if;
      ...
      begin
         if Program'Length <= 0 then
            ...
            if Tash.Regexp.Match (Raw_Line (1 .. Last), RE_Bourne_Shell) then
               ...
               if Print_Why then
                  Ada.Text_IO.Put_Line
                    (" because it has no extension and" &
   ...
   for I in  1 .. Ada.Command_Line.Argument_Count loop
      ...
      begin
         if Arg = "-verbose" then
            ...
            while not Ada.Text_IO.End_Of_File (Ada.Text_IO.Standard_Input)
            loop
               Ada.Text_IO.Get_Line (Item => Line, Last => Last);
