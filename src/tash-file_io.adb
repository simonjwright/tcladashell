...
--   end Put_Objv;
   ...
   function To_File_Mode_List (Mode : in File_Mode_List)
      return String is
   --
   ...
   begin -- To_File_Mode_List
      for I in Mode'Range loop
         Ada.Strings.Unbounded.Append (
            Mode_String, " " & File_Mode'Image (Mode ((I))));
      end loop;
      ...
      begin
         ...
         if Result = Tcl.TCL_ERROR then
            Tash_Interp.Raise_Exception (
               Interp, Tash.Tcl_Error'Identity, Result_String);
         end if;
         ...
         File := (Ada.Finalization.Controlled with
              Obj  => New_Obj);
      end;
   ...
   function End_Of_File (
      File : in Tash_File) return Boolean is
   ...
   begin -- Get
      ...
      begin
         ...
         Tcl.Tcl_ResetResult (Interp);
         Tash_Interp.Release (Interp);
         Item (Item'first..Item'First+Result_String'Length-1) := Result_String;
      end;
   ...
   begin -- Get_Line
      ...
      begin
         ...
         if Result_String'Length <= Item'Length then
            Last := Item'First + Result_String'Length - 1;
   ...
   function Get_Buffering_Mode (
      File : in Tash_File) return Buffering_Mode is
   ...
   function Get_Buffer_Size (
      File : in Tash_File) return Buffer_Size is
   ...
   function Get_Encoding (
      File     : in Tash_File) return Encoding_Type is
   ...
   begin -- Get_Encoding
      ...
      begin
         ...
         exception
            when Constraint_Error => return Normal;
         end;
      end;
   ...
   procedure Set_Encoding (
      File     : in Tash_File;
      Encoding : in Encoding_Type) is
   begin -- Set_Encoding
      null;
   end Set_Encoding;

   function Get_EOF_Char (
      File     : in Tash_File) return String is
   ...
   begin -- Get_EOF_Char
      ...
      declare
         Result_String : constant String := CHelper.Value (
             Tcl.Tcl_GetString (ListElem.all));
       begin
          Tcl.Tcl_ResetResult (Interp);
          Tash_Interp.Release (Interp);
   ...
   function Get_EOF_Char (
      File : in Tash_File) return Character is
   --
   ...
   function EOF_Char_Is_Empty_String (
      File : in Tash_File) return Boolean is
   ...
   procedure Set_EOF_Char_To_Empty_String (
      File : in Tash_File) is
   ...
   procedure Set_EOF_Char (
      File     : in Tash_File;
      In_Char  : in String;
      Out_Char : in String) is
   begin -- Set_EOF_Char
      null;
   end Set_EOF_Char;

   function Get_Translation_Mode (
      File : in Tash_File) return Translation_Mode is
   ...
   procedure Get_Translation_Mode (
      ...
      Out_Mode : out Translation_Mode) is
   begin -- Get_Translation_Mode
      In_Mode  := Auto;
      Out_Mode := Auto;
   end Get_Translation_Mode;
   ...
   procedure Set_Translation_Mode (
      ...
      Out_Mode : in Translation_Mode) is
   begin -- Set_Translation_Mode
      null;
   end Set_Translation_Mode;
