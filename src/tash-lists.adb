...
package body Tash.Lists is
   ...
   function "&" (
      Left  : in Tash_List;
      Right : in Tash_List) return Tash_List is
   --
      New_List : Tash_List;
   begin -- "&"
      New_List := Copy (Left);
   ...
   function "&" (
      Left  : in Tash_List;
      Right : in String) return Tash_List is
   --
      New_List : Tash_List;
   begin -- "&"
      New_List := Copy (Left);
   ...
   function Copy (
      List   : in Tash_List) return Tash_List is
   ...
   begin -- Copy
      ...
      Tash_Interp.Get (Interp);
      for I in 1..Len loop
         Obj_Ptr := Tash.Lists.Get_Element (
            Interp => Interp,
            List   => List.Obj,
            Index  => I);
         Result := Tcl.Tcl_ListObjAppendElement (
   ...
   function To_String (
      List : in Tash_List) return String is
   --
   ...
   function Join (
      List       : in Tash_List;
      JoinString : in String := " ") return String is
   ...
   function Get_Element (
      List  : in Tash_List;
      Index : in Positive) return Tash_List is
   --
      Interp : Tcl.Tcl_Interp;
   begin -- Get_Element
      if Is_Null (List) or else Index > Length (List) then
   ...
   begin -- Element_Is_List
      Tash_Interp.Get (Interp);
      Result := Type_Of_List_Element (
         Get_Element (Interp, List.Obj, Index)) = "list";
      Tash_Interp.Release (Interp);
      return Result;
   exception
      when others => return False;
   ...
   begin -- Replace_Element_With_List
      Replace_Slice_With_List (List    => List,
                               From    => Index,
                               To      => Index,
                               Element => Element);
   ...
   begin -- Replace_Element_With_Elements
      Replace_Slice_With_Elements (List     => List,
                                   From     => Index,
                                   To       => Index,
                                   Elements => Elements);
   ...
   package body Generic_Integer_Lists is
      ...
      function To_Tash_List (
         Num : in Item) return Tash_List is
      ...
      function "&" (
         Left  : in Tash_List;
         Right : in Item) return Tash_List is
      --
         New_List : Tash_List;
      begin -- "&"
         New_List := Copy (Left);
      ...
      begin -- Element_Is_Integer
         Tash_Interp.Get (Interp);
         Result := Type_Of_List_Element (
            Get_Element (Interp, List.Obj, Index)) = "int";
         Tash_Interp.Release (Interp);
         return Result;
      exception
         when others => return False;
      ...
      function Head (
         List : in Tash_List) return Item is
      begin -- Head
         return Get_Element (List, 1);
      end Head;
   ...
   package body Generic_Float_Lists is
      ...
      begin -- Element_Is_Float
         Tash_Interp.Get (Interp);
         Result := Type_Of_List_Element (
            Get_Element (Interp, List.Obj, Index)) = "double";
         Tash_Interp.Release (Interp);
         return Result;
      exception
         when others => return False;
   ...
   begin -- Format
      ...
      begin
         ...
         for I in 1..Length (Interp, Values.Obj) loop
            Obj_Ptr := Tash.Lists.Get_Element (Interp => Interp,
                                           List   => Values.Obj,
                                           Index  => Integer (I));
